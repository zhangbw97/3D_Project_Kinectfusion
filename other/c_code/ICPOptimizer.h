...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		T outputPoint[3];
		T inputPoint[3] = { T(m_sourcePoint[0]),  T(m_sourcePoint[1]), T(m_sourcePoint[2]) };
		poseIncrement.apply(inputPoint, outputPoint);


		residuals[0] = T(sqrt(m_weight)) * (outputPoint[0] - T(m_targetPoint[0])) * T(LAMBDA);
		residuals[1] = T(sqrt(m_weight)) * (outputPoint[1] - T(m_targetPoint[1])) * T(LAMBDA);
		residuals[2] = T(sqrt(m_weight)) * (outputPoint[2] - T(m_targetPoint[2])) * T(LAMBDA);
...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		poseIncrement.apply(inputPoint, outputPoint);
		T temp[3];
		temp[0] = T(m_targetNormal[0]) * (outputPoint[0] - T(m_targetPoint[0]));
		temp[1] = T(m_targetNormal[1]) * (outputPoint[1] - T(m_targetPoint[1]));
		temp[2] = T(m_targetNormal[2]) * (outputPoint[2] - T(m_targetPoint[2]));

		residuals[0] =  T(sqrt(m_weight)) * (temp[0] + temp[1] + temp[2]) * T(LAMBDA);
		return true;
	}
	...
	void pruneCorrespondences(const std::vector<Vector3f>& sourceNormals, const std::vector<Vector3f>& targetNormals, std::vector<Match>& matches) {
		...
		for (unsigned i = 0; i < nPoints; i++) {
			...
			if (match.idx >= 0) {
				...
				auto angle = acos(targetNormal.dot(sourceNormal));
				if (angle > (3.14/3.0))
					{	
						match.idx = -1;
					}
					// 直接求点乘
				// if (targetNormal.dot(sourceNormal)<0.5)
				// {
				// 	match.idx = -1;
				// }
	...
	void prepareConstraints(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals, const std::vector<Match> matches, const PoseIncrement<double>& poseIncrement, ceres::Problem& problem) const {
		...
		for (unsigned i = 0; i < nPoints; ++i) {
			...
			if (match.idx >= 0) {
				...
				ceres::CostFunction* constraint1 = PointToPointConstraint::create(sourcePoint,targetPoint,weight);
				...
				if (m_bUsePointToPlaneConstraints) {
					...
					// to the Ceres problem.
					ceres::CostFunction* constraint2 = PointToPlaneConstraint::create(sourcePoint, targetPoint, targetNormal, weight);
					problem.AddResidualBlock(
								constraint2,
								nullptr, poseIncrement.getData()
							);
...
private:
	...
	Matrix4f estimatePosePointToPlane(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals) {
		...
		for (unsigned i = 0; i < nPoints; i++) {
			...
			float ai1 = n[2] * s[1] - n[1] * s[2];
			float ai2 = n[0] * s[2] - n[2] * s[0];
			float ai3 = n[1] * s[0] - n[0] * s[1];
			A.row(index) << ai1, ai2, ai3, n[0], n[1], n[2];
			b[index] = n[0] * d[0] + n[1] * d[1] + n[2] * d[2] - (n[0] * s[0] + n[1] * s[1] + n[2] * s[2]);
			// TODO: Add the point-to-point constraints to the system
			A.row(index+1) << 0,s[2],-s[1], 1.0, 0, 0;
			b[index+1] = d[0] - s[0];
			A.row(index+2) << -s[2],0,s[0], 0, 1.0, 0;
			b[index+2] = d[1]-s[1];
			A.row(index+3) << s[1],-s[0], 0, 0, 0, 1.0;
			b[index+3] = d[2]-s[2];
			...
			A.block(index,0,1,6) *= pointToplaneWeight;
			b[index] *= pointToplaneWeight;
			A.block(index+1,0,3,6) *= pointTopointWeight;
			b.segment(index+1,3) *= pointTopointWeight;
		}
