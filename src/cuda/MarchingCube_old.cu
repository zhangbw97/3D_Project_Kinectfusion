...
namespace cuda {
    ...
    //##### KERNELS #####
    __global__
    void get_occupied_voxels_kernel(const PtrStep<float2> volume, const int3 volume_size,
    ...
    __global__
    void generate_triangles_kernel(const PtrStep<float2> tsdf_volume, const int3 volume_size, const float voxel_size,
    ...
    Mesh extract_mesh(const VolumeData& volume, const int triangles_buffer_size)
    {
        MeshData mesh_data(triangles_buffer_size / 3);

        GpuMat number_vertices_table1, triangle_table1;

        number_vertices_table1 = cv::cuda::createContinuous(256, 1, CV_32SC1);
        number_vertices_table1.upload(cv::Mat(256, 1, CV_32SC1, number_vertices_table_host, cv::Mat::AUTO_STEP));

        triangle_table1 = cv::cuda::createContinuous(256, 16, CV_32SC1);
        triangle_table1.upload(cv::Mat(256, 16, CV_32SC1, triangle_table_host, cv::Mat::AUTO_STEP));
        ...
        generate_triangles_kernel<<<grid, block>>> (volume.tsdf_volume,
                ...
                number_vertices_table1, triangle_table1,
        ...
        return Mesh { vertex_output, color_output, total_vertices, total_vertices / 3 };
    }
}