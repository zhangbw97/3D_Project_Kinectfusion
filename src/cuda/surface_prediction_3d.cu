...
namespace cuda {
    ...
    float tri_interpolate_3d(const float& x, const float& y, const float& z, const PtrStep<float2>& tsdf_volume,
    ...
    __device__ __forceinline__
    float get_min_time_3d(const float3& volume_boundary, const Vec3fda& origin, const Vec3fda& direction)
    {
    ...
    __device__ __forceinline__
    float get_max_time_3d(const float3& volume_boundary, const Vec3fda& origin, const Vec3fda& direction)
    {
    ...
    __global__
    void raycast_tsdf_3d_kernel(const PtrStepSz<float2> tsdf_volume, const PtrStepSz<uchar3> color_volume,
                             ...
                             const Vec3fda translation)
    {
        ...
        float ray_length_back = fmaxf(get_min_time_3d(volume_boundary, translation, ray_direction), 0.f);
        // float ray_length_front = get_max_time_3d(volume_boundary, translation, ray_direction);
        ...
        Vec3fda grid = (translation + (ray_direction * (ray_length_back + z * step))) / voxel_scale;
        ...
        if (grid_prev.x() < 1 || grid_prev.x() >= volume_size.x - 1 || grid_prev.y() < 1 ||
                grid_prev.y() >= volume_size.y - 1 ||
                grid_prev.z() < 1 || grid_prev.z() >= volume_size.z - 1)
                return;


        float tsdf_prev = tsdf_volume.ptr(
                __float2int_rd(grid_prev.z()) * volume_size.y + __float2int_rd(grid_prev.y()))[__float2int_rd(grid_prev.x())].x;
        ...
        if (tsdf_prev > 0.f) {
            ...
            const float t_star =
                    ray_length_back + (z - 1) * step - step * tsdf_prev / (tsdf - tsdf_prev);
            ...
            const float Fx1 = tri_interpolate_3d(vx + 1, vy, vz, tsdf_volume, volume_size);
            const float Fx2 = tri_interpolate_3d(vx - 1, vy, vz, tsdf_volume, volume_size);

            const float Fy1 = tri_interpolate_3d(vx, vy + 1, vz, tsdf_volume, volume_size);
            const float Fy2 = tri_interpolate_3d(vx, vy - 1, vz, tsdf_volume, volume_size);

            const float Fz1 = tri_interpolate_3d(vx, vy, vz + 1, tsdf_volume, volume_size);
            const float Fz2 = tri_interpolate_3d(vx, vy, vz - 1, tsdf_volume, volume_size);
                            ...
                            const Eigen::Matrix4f& pose)
    {
        ...
        dim3 blocks((model_vertex.cols + threads.x - 1) / threads.x,
                    ...
                    (max_step_grid + threads.z - 1) / threads.z);

        // We use parallel algorithm instead of iteration when calculating raycast

        raycast_tsdf_3d_kernel<<<blocks, threads>>>(volume.tsdf_volume, volume.color_volume,
