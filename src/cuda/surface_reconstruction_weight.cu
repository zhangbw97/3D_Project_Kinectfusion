...
namespace cuda {
    ...
    __global__
    void update_tsdf_kernel(const PtrStepSz<float> depth_image, const PtrStepSz<float3> normal_image,
                            ...
                            Eigen::Matrix<float, 3, 3, Eigen::DontAlign> rotation, Vec3fda translation)
    {
        ...
        if (sdf >= (-1.f) * truncation_distance) {
            ...
            const float current_weight = current_tsdf_tuple.y;

            // TODO: add weight
            const float3 current_normal = normal_image.ptr(uv.y())[uv.x()];
            float numerator = depth;
            if (current_normal.z != 0)
                numerator = weight_factor / lambda *
                (current_normal.x * qlambda.x() + current_normal.y * qlambda.y() + current_normal.z * qlambda.z());
            const float add_weight = numerator / depth;
    ...
    void surface_reconstruction(const cv::cuda::GpuMat& depth_image, const cv::cuda::GpuMat& normal_image,
                                ...
                                const Eigen::Matrix4f& extrinsic)
    {
        ...
        update_tsdf_kernel<<<blocks, threads>>>(depth_image, normal_image, color_image,
                volume.tsdf_volume, volume.color_volume,
                volume.volume_size, volume.voxel_scale,
                cam_params, truncation_distance, iso_level, weight_factor,
