...
struct SurfaceMesh {
	...
	bool writeMesh(const std::string& filename) {
		...
		outFile << num_vertices << " " << num_triangles << " 0" << std::endl;

		// Save vertices.
		for (unsigned int i = 0; i < num_vertices; i++) {
			float3 vertex = triangles.ptr<float3>(0)[i];
			uchar3 color = colors.ptr<uchar3>(0)[i];
			//const auto& vertex = m_vertices[i];
			outFile << vertex.x << " " << vertex.y << " " << vertex.z << " ";
            outFile << (int) color.z << " " << (int) color.y << " " << (int) color.x << " 255" << std::endl;
		}

		// Save faces.
		for (int t_idx = 0; t_idx < num_vertices; t_idx += 3) {
			outFile << 3 << " " << t_idx + 1 << " " << t_idx << " " << t_idx + 2 << std::endl;
		}
	...
	SimpleMesh(VirtualSensor& sensor, const Matrix4f& cameraPose, float edgeThreshold = 0.01f) {
		...
		for (unsigned int v = 0; v < sensor.getDepthImageHeight(); ++v) {
			...
			for (unsigned int u = 0; u < sensor.getDepthImageWidth(); ++u) {
				...
				if (depth == 0.f) {
					m_vertices[idx].position = Vector4f(0.f, 0.f, 0.f, 0.f);
